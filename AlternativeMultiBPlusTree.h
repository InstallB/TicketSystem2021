...
namespace RainyMemory {
    ...
    private:
        ...
        LRUCacheMemoryPool<internalNode, int> * internalPool = nullptr;
        basicInfo info {};
    ...
    private:
        ...
        public:
            void addElement(AlternativeMultiBPlusTree * tree, const key &o1, const data &o2) {
                int pos = std::upper_bound(leafKey, leafKey + dataNumber, o1) - leafKey;
            ...
            bool deleteElement(AlternativeMultiBPlusTree * tree, const key &o) {
                int pos = std::lower_bound(leafKey, leafKey + dataNumber, o) - leafKey;
            ...
            bool updateElement(AlternativeMultiBPlusTree * tree, const key &o1, const data &newData) {
                int start = std::lower_bound(leafKey, leafKey + dataNumber, o1) - leafKey;
                int end = std::upper_bound(leafKey, leafKey + dataNumber, o1) - leafKey;
                bool result = false;
    ...
    private:
        ...
        insertReturn recursionInsert(int now, const key &o1, const data &o2) {
            ...
            if (nowNode.childNodeIsLeaf) {
                int index = std::upper_bound(nowNode.nodeKey, nowNode.nodeKey + nowNode.keyNumber, o1) - nowNode.nodeKey;
        ...
        eraseReturn recursionErase(int now, const key &o) {
            internalNode nowNode {internalPool->read(now)};
            if (nowNode.childNodeIsLeaf) {
                int index = std::upper_bound(nowNode.nodeKey, nowNode.nodeKey + nowNode.keyNumber, o) - nowNode.nodeKey;
                leafNode targetNode {leafPool->read(nowNode.childNode[index])};
                eraseReturn temp;
        ...
        void recursionFind(int now, const key &o, sjtu::vector <data> &result) const {
            internalNode nowNode = internalPool->read(now);
            if (nowNode.childNodeIsLeaf) {
                int index = std::upper_bound(nowNode.nodeKey, nowNode.nodeKey + nowNode.keyNumber, o) - nowNode.nodeKey;
                ...
                while (cur >= 0 && flag) {
                    ...
                    int pos = std::upper_bound(targetNode.leafKey, targetNode.leafKey + targetNode.dataNumber, o) - targetNode.leafKey;
    ...
    public:
        explicit AlternativeMultiBPlusTree(const string &name) :
                ...
                internalPool(new LRUCacheMemoryPool<internalNode, int>("Internal" + name, int {}, CACHE_SIZE)),
        ...
        void insert(const mainKey &k1, const assistantKey &k2, const data &o2) {
            ...
            if (rootNode.childNodeIsLeaf) {
                int index = std::upper_bound(rootNode.nodeKey, rootNode.nodeKey + rootNode.keyNumber, o1) - rootNode.nodeKey;
                ...
                if (rootNode.keyNumber == MAX_KEY_NUM)rootNode.splitRoot(this), updateRoot = false;
            } else {
                int index = std::upper_bound(rootNode.nodeKey, rootNode.nodeKey + rootNode.keyNumber, o1) - rootNode.nodeKey;
            ...
            if (updateRoot) internalPool->update(rootNode, info.root);
        }
        ...
        bool erase(const mainKey &k1, const assistantKey &k2) {
            ...
            bool deleted;
            if (rootNode.childNodeIsLeaf) {
                int index = std::upper_bound(rootNode.nodeKey, rootNode.nodeKey + rootNode.keyNumber, o) - rootNode.nodeKey;
                leafNode targetNode {leafPool->read(rootNode.childNode[index])};
            ...
            if (deleted)info.size--, updInfo();
            internalPool->update(rootNode, info.root);
            return deleted;
        }
        
        void find(const mainKey &m, sjtu::vector <data> &result) const {
